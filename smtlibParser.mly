...
%token <string> BVBIN
%token <string> BVHEX
%token <string> SMTLIBOPTION
%token LPAREN RPAREN COLON EOF SETLOGIC SETOPTION TRUE FALSE NOT AND OR IMPL XOR DISTINCT EQUALS ITE BOOL 
%token DECLARECONST DECLAREFUN DECLARESORT DEFINEFUN CHECKSAT EXIT LET ASSERT ARRAY SELECT STORE
%token BITVEC INDEX BVAND BVOR BVXOR BVNAND BVNOR BVXNOR BVMUL BVADD BVSUB BVUDIV BVUREM HASH_SEMI
...
command:
  ...
  | LPAREN SETOPTION SMTLIBOPTION sorted_term RPAREN { Zilch () }
